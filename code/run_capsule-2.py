...
def calculate_metrics(true_mask, masks_pred):
    ...
    # Calculate intersection and union for Dice and IoU scores
    ...
    # Calculate number of ROIs (Regions of Interest)
    ...
    flattened_true = true_mask.flatten()
    flattened_pred = masks_pred.flatten()
    precision = precision_score(flattened_true, flattened_pred, average=None, zero_division=0)
    recall = recall_score(flattened_true, flattened_pred, average=None, zero_division=0)
    f1 = f1_score(flattened_true, flattened_pred, average=None, zero_division=0)
...
def main(model_name: str, retrain: bool):
    ...
    if retrain:
        ...
        # Define ranges for thresholds
        cellprob_thresholds = np.arange(-6, 7, 2)  # Range from -6 to 6
        flow_thresholds = np.arange(0.1, 3.1, 0.2)  # Range from 0.1 to 3.0
