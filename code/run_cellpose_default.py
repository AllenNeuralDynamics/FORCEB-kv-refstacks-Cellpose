...
def process_image(i, image, true_mask, model, cellprob_thresholds, flow_thresholds, model_save_path, model_name):
    results_list = []
    for cellprob_threshold in cellprob_thresholds:
        for flow_threshold in flow_thresholds:
            results = model.eval(image, cellprob_threshold=cellprob_threshold, flow_threshold=flow_threshold)
            if len(results) == 3:
...
def main(model_name: str, retrain: bool, gpu_id):
    ...
    if not retrain:
        ...
        with ThreadPoolExecutor() as executor:
            futures = {executor.submit(process_image, i, image, true_mask, model, cellprob_thresholds, flow_thresholds, model_save_path, model_name): i 
        ...
        with ThreadPoolExecutor() as executor:
            futures = {executor.submit(process_image, i, test_images, test_masks, model, cellprob_thresholds, flow_thresholds, model_save_path, model_name): i 
                    for i, (test_images, test_masks) in enumerate(zip(test_images, test_masks))}
